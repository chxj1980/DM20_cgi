...
int dq_string_get(char *word_buf, unsigned int word_buf_len, char *input_buf, unsigned int *offset)
{
	...
	if (input_buf[cur_offset] == '"')
	{
		...
		{
			log_print(LOG_WARN, "asn_dq_string_get::string len %d > max len\r\n",dq_str_len);
			return -1;
		}
...
ONVIF_API BOOL is_char(char ch)
{
...
ONVIF_API BOOL is_separator(char ch)
{
	unsigned int i;
...
ONVIF_API BOOL GetLineText(char *buf,int cur_line_offset,int max_len,int * len,int * next_line_offset)
{
...
ONVIF_API BOOL GetSipLine(char *p_buf, int max_len, int * len, BOOL * bHaveNextLine)
{
...
ONVIF_API BOOL GetLineWord(char *line,int cur_word_offset,int line_max_len,char *word_buf,int buf_len,int *next_word_offset,WORD_TYPE w_t)
{
	int len;
...
ONVIF_API BOOL GetNameValuePair(char * text_buf,int text_len,const char * name,char *value,int value_len)
{
	char word_buf[256];
...
ONVIF_API BOOL is_ip_address(const char * address)
{	
	int i;
	int index = 0;
...
ONVIF_API BOOL is_integer(char * p_str)
{
	int i;
