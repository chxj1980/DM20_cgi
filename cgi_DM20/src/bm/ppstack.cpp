...
ONVIF_API PPSN_CTX * pps_ctx_fl_init_assign(char * mem_addr,unsigned long mem_len,unsigned long node_num,unsigned long content_size,BOOL bNeedMutex)
{
	...
	{
		log_print(LOG_ERR, "pps_ctx_fl_init_assign:: assign mem len too short!!!\r\n");
		return NULL;
	}
...
ONVIF_API BOOL ppstack_fl_push(PPSN_CTX * pps_ctx,void * content_ptr)
{
	...
	{
		log_print(LOG_WARN, "ppstack_push::unit ptr error!!!\r\n");
		return FALSE;
	}
	...
	{
		log_print(LOG_WARN, "ppstack_push::unit node %d already in freelist !!!\r\n",pps_get_index(pps_ctx, content_ptr));
...
ONVIF_API BOOL ppstack_fl_push_tail(PPSN_CTX * pps_ctx,void * content_ptr)
{
	...
	if(pps_safe_node(pps_ctx, content_ptr) == FALSE)
	{
		log_print(LOG_WARN, "ppstack_fl_push_tail::unit ptr error!!!\r\n");
		return FALSE;
	}
	...
	if(p_node->node_flag == 1)
	{
		log_print(LOG_WARN, "ppstack_fl_push_tail::unit node %d already in freelist !!!\r\n",pps_get_index(pps_ctx, content_ptr));
		pps_post_mutex(pps_ctx);
		return FALSE;
	}
...
ONVIF_API PPSN * pps_ctx_ul_del_node_unlock(PPSN_CTX * ul_ctx,PPSN * p_node)
{
	...
	{
		log_print(LOG_WARN, "pps_ctx_ul_del_node_unlock::unit not in used list!!!\r\n");
		return NULL;
	}

	if(ul_ctx->head_node == 0)
	{
		log_print(LOG_WARN, "pps_ctx_ul_del_node_unlock::used list is empty!!!\r\n");
		return NULL;
	}
...
PPSN * _pps_node_next(PPSN_CTX * pps_ctx, PPSN * p_node)
{
	...
	{
		log_print(LOG_WARN, "pps_lookup_next::unit ptr error!!!!!!\r\n");
		return NULL;
	}
...
ONVIF_API void * pps_lookup_next(PPSN_CTX * pps_ctx, void * ctx_ptr)
{
	PPSN * p_node;
...
ONVIF_API void * pps_lookback_next(PPSN_CTX * pps_ctx, void * ctx_ptr)
{
	PPSN * p_node;
...
ONVIF_API uint32 pps_get_index(PPSN_CTX * pps_ctx,void * content_ptr)
{
	...
	{
		log_print(LOG_WARN, "pps_get_index::unit ptr error!!!\r\n");
		return 0xFFFFFFFF;
	}
	...
	if(offset != 0)
	{
		...
		log_print(LOG_WARN, "pps_get_index::unit ptr error,pps_ctx[0x%08x],ptr[0x%08x],low_offset[0x%08x],offset[0x%08x],like entry[%u]\r\n",
...
ONVIF_API void * pps_get_node_by_index(PPSN_CTX * pps_ctx,unsigned long index)
{
	...
	{
		if(index != 0xFFFFFFFF)
			log_print(LOG_WARN, "pps_get_node_by_index::index [%u]error!!!\r\n",index);
		return NULL;
	}
...
ONVIF_API void pps_wait_mutex(PPSN_CTX * pps_ctx)
{
	if(pps_ctx == NULL)
	{
		log_print(LOG_WARN, "pps_wait_mutex::pps_ctx == NULL!!!\r\n");
		return;
	}
...
ONVIF_API void pps_post_mutex(PPSN_CTX * pps_ctx)
{
	if(pps_ctx == NULL)
	{
		log_print(LOG_WARN, "pps_post_mutex::pps_ctx == NULL!!!\r\n");
		return;
	}
...
ONVIF_API BOOL pps_safe_node(PPSN_CTX * pps_ctx,void * content_ptr)
{
	...
	if ((char *)content_ptr < (pps_ctx->low_offset + pps_ctx->fl_base) ||
		(char *)content_ptr > (pps_ctx->high_offset + pps_ctx->fl_base))
	{
		// log_print(LOG_WARN, "pps_safe_node::unit ptr error!!!\r\n");
		return FALSE;
	}
	...
	if(offset != 0)
	{
		...
		log_print(LOG_WARN, "pps_safe_node::unit ptr error,pps_ctx[0x%08x],ptr[0x%08x],low_offset[0x%08x],offset[0x%08x],like entry[%u]\r\n",
			pps_ctx,content_ptr,pps_ctx->low_offset,offset,index);
		return FALSE;
	}
...
ONVIF_API BOOL pps_used_node(PPSN_CTX * pps_ctx,void * content_ptr)
{
	...
	if(pps_ctx->head_node == 0)	
	{
		log_print(LOG_WARN, "pps_used_node::used list is empty!!!\r\n");
		return FALSE;
	}
