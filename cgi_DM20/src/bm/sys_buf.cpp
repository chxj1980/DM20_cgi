...
ONVIF_API BOOL hdrv_buf_fl_init(int num)
{
	...
	log_print(LOG_INFO, "hdrv_buf_fl_init::num = %lu\r\n",hdrv_buf_fl->node_num);
...
//	log_print(LOG_INFO, "get_hdrv_buf::num = %d\r\n",hdrv_buf_fl->node_num);
//	pps_ctx_fl_show(hdrv_buf_fl);
	return p_ret;
}
...
//	log_print(LOG_INFO, "free_hdrv_buf::num = %d,pHdrv=0x%08x\r\n",hdrv_buf_fl->node_num,pHdrv);
//	pps_ctx_fl_show(hdrv_buf_fl);
}
...
ONVIF_API BOOL sys_buf_init()
{
	...
	{
		log_print(LOG_ERR, "sys_buf_init::net_buf_fl_init failed!!!\r\n");
		return FALSE;
	}

	if (hdrv_buf_fl_init(1024) == FALSE)
	{
		log_print(LOG_ERR, "sys_buf_init::hdrv_buf_fl_init failed!!!\r\n");
		return FALSE;
	}
