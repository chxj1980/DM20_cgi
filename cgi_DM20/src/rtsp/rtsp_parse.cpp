...
int rtsp_line_parse(char * p_buf, int max_len, char sep_char, PPSN_CTX * p_ctx)
{
	...
	do{
		...
		{
			// log_print(LOG_ERR, "rtsp_line_parse::get sip line error!!!\r\n");
			return -1;
		}
		...
		{
			log_print(LOG_ERR, "rtsp_line_parse::format error!!!\r\n");
			return -1;
		}
		...
		if(pHdrV == NULL)
		{
			log_print(LOG_ERR, "rtsp_line_parse::get_hdrv_buf return NULL!!!\r\n");
			return -1;
		}
...
BOOL get_rtsp_headline_string(HRTSP_MSG * rx_msg,char * head, char * p_value, int size)
{
	...
	if(len >= size)
	{
		log_print(LOG_ERR, "get_rtsp_headline_string::%s, value_string(%s) len(%d) > size(%d)\r\n",
...
BOOL get_rtsp_session_info(HRTSP_MSG * rx_msg,char * session_buf,int max_len, int * timeout)
{
...
{
	HDRV * rx_id = find_rtsp_headline(rx_msg,"Public");
	if(rx_id == NULL)
		return FALSE;
...
void add_rtsp_tx_msg_line(HRTSP_MSG * tx_msg,const char * msg_hdr,const char * msg_fmt,...)
{
	...
	if(pHdrV == NULL)
	{
		log_print(LOG_ERR, "add_tx_msg_line::get_hdrv_buf return NULL!!!\r\n");
		return;
	}
	...
	if(slen < 0)
	{
		log_print(LOG_ERR, "add_tx_msg_line::vsnprintf return %d !!!\r\n",slen);
		free_hdrv_buf(pHdrV);
		return;
	}
...
void add_rtsp_tx_msg_sdp_line(HRTSP_MSG * tx_msg,const char * msg_hdr,const char * msg_fmt,...)
{
	...
	if(pHdrV == NULL)
	{
		log_print(LOG_ERR, "add_tx_msg_sdp_line::get_hdrv_buf return NULL!!!\r\n");
		return;
	}
	...
	if(slen < 0)
	{
		log_print(LOG_ERR, "add_tx_msg_sdp_line::vsnprintf return %d !!!\r\n",slen);
		free_hdrv_buf(pHdrV);
		return;
	}
...
void add_rtsp_tx_msg_fline(HRTSP_MSG * tx_msg,const char * msg_hdr,const char * msg_fmt,...)
{
	...
	if(slen < 0)
	{
		log_print(LOG_ERR, "add_tx_msg_fline::vsnprintf return %d !!!\r\n",slen);
		return;
	}
...
BOOL get_rtsp_remote_media_ip(HRTSP_MSG * rx_msg,unsigned long * media_ip)
{
	...
	if((pHdr != NULL) && (pHdr->value_string != NULL) && (strlen(pHdr->value_string) > 0))
	{
		...
		log_print(LOG_DBG, "get_remote_media_ip::media_ip=%s\r\n",tmp_buf);

		if(is_ip_address(tmp_buf))
		{
			*media_ip = inet_addr(tmp_buf);
		//	log_print(LOG_DBG, "get_remote_media_ip::media_ip=0x%08x\r\n",*media_ip);
			return TRUE;
		}
	}
