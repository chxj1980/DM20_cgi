                           ...
                           CHAR *pPassword)         // [IN] User password for proxy authentication (can be null)
{
    ...
    {
        pHTTPSession->HttpProxy.ProxyAuthSchema = AuthSchemaBasic;//HTTP_CLIENT_DEFAULT_PROXY_AUTH;   
    }
    return HTTP_CLIENT_SUCCESS;
}
                            ...
                            // Can be used for evaluating a HEAD request
{
    ...
    do
    {
        ...
        /* put [verb] [URI] [HTTP] together */
        memset(Allinone, 0x00, sizeof(Allinone));
    ...
    #else
        strcpy(Allinone, pHTTPSession->HttpHeaders.Verb);
        strcat(Allinone, " ");
    #endif
        ...
        #else
            strcat(Allinone, pHTTPSession->HttpUrl.UrlRequest.pParam);
        #endif
            // Set the counters
            pHTTPSession->HttpCounters.nSentHeaderBytes += nBytes;
        }
        ...
        #else
            strcat(Allinone, pHTTPSession->HttpUrl.Url);
        #endif
            // Set the counters
            pHTTPSession->HttpCounters.nSentHeaderBytes += nBytes;
        }
    ...
    #else
        strcat(Allinone, RequestCmd);
    #endif
        // Set the counters
        pHTTPSession->HttpCounters.nSentHeaderBytes += nBytes;

    #ifndef SEND_SEPRATE
        nBytes = pHTTPSession->HttpCounters.nSentHeaderBytes;
        if((nRetCode = HTTPIntrnSend(pHTTPSession, Allinone, &nBytes)) != HTTP_CLIENT_SUCCESS)  
        {
            break;
        }
    #endif    
